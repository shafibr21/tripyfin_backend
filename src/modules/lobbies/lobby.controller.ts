...
const getLobbySummary = async (req: Request, res: Response) => {
  try {
    const user = req.user
    const idRaw = req.params.id
    const id = Array.isArray(idRaw) ? idRaw[0] : idRaw
    ...
    const result = await lobbyService.getLobbySummary(id, user.id)
    ...
    console.error("Get lobby summary error:", error)
    return res.status(500).json({ message: "Internal server error" })
  }
};
